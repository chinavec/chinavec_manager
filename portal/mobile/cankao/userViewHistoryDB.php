<?php
	...
	$videoSql = "SELECT COUNT(*) FROM `video` WHERE `video`.`id` = '$videoID'"; //得出id = $videoID的视频的总数.
	...
	if ($totalResult){
	    if(userViewHistoryUpdate($userID,$videoID,$viewTime,$viewProgress,$db)){
		    echo json_encode(userViewHistoryUpdate($userID,$videoID,$viewTime,$viewProgress,$db));   
            systemLog('更新数据库中user_view_history表成功', 1, 2, $db); //用日志记录数据表执行成功.			 
		}
		else {
		    echo false;
			systemLog('更新数据库中user_view_history表失败', 1, 5, $db); //用日志记录数据表执行失败.
		}
	}
	else {
	    if ($videoResult) {
		    echo json_encode(userViewHistoryInsert($userID,$videoID,$viewTime,$viewProgress,$db));
		    systemLog('插入数据库中user_view_history表成功', 1, 2, $db); //用日志记录数据表执行成功.	
		}
		else {
		    echo false;
			systemLog('插入数据库中user_view_history表失败', 1, 5, $db); //用日志记录数据表执行失败.
		}
	}
?>