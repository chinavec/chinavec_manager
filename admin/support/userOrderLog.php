...
if($q == '') {
//读取订购数据表user_business
$sqlCount = "SELECT COUNT(*) FROM `user_business` WHERE `subscribe_time` BETWEEN $s AND $e";
$sql = "SELECT `user`.`username`, `business`.`name`, `user_business`.`user_id`, 
		`user_business`.`business_id`, `user_business`.`subscribe_time`
		FROM `user_business` 
		JOIN `user` ON `user`.`id` = `user_business`.`user_id` 
		JOIN `business` ON `business`.`id` = `user_business`.`business_id`
		WHERE `user_business`.`subscribe_time` BETWEEN $s AND $e
		ORDER BY `user_business`.`subscribe_time` DESC
		LIMIT $offset,$pageSize";
}else{
$sqlCount = "SELECT COUNT(*) FROM `user_business`
			JOIN `user` ON `user`.`id` = `user_business`.`user_id` 
			JOIN `business` ON `business`.`id` = `user_business`.`business_id`
			WHERE (`user`.`username` LIKE '%$q%' OR `business`.`name` LIKE '%$q%')
			AND `subscribe_time` BETWEEN $s AND $e";
$sql = "SELECT `user`.`username`, `business`.`name`, `user_business`.`user_id`, 
		`user_business`.`business_id`, `user_business`.`subscribe_time`
		FROM `user_business` 
		JOIN `user` ON `user`.`id` = `user_business`.`user_id` 
...
//设置url的形式
$basicURL = "userOrderLog.php?q=$q&s={$_s}&e={$_e}";
//关闭数据库
$db->close();
?>
...
<style>
	#container_left ul li.userOrderLog{background-color:#108dbd;}
	#container_left ul li.userOrderLog a{color:#FFF;}
</style>
<script type="text/javascript">
	$(function(){
			$("<img src='../../img/lbg.gif'></img>").appendTo("#container_left ul li.user-order-log");
		});
</script>
</head>
                    ...
                    <div class="search">
                        <div class="search_left">
                            <form action="" method="get">
                                ...
                                <a href="userOrderLog.php">清除搜索条件</a>
                    ...
                    <?php foreach($res as $key => $item): ?>
                        <li class="log_list">
                            <div>
                                <div class="log_title">
                                    <p id="log" class="log">
                                        <span class="date">【<?php echo date('Y-m-d',$item->subscribe_time); ?>】</span>
                                        <?php echo $item->username.'订购了'.$item->name.'业务。'; ?>
                                    </p>
                                </div>
                            </div>
                        </li>
